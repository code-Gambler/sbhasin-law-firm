    ...
    <nav className='lg:hidden relative z-20'>
      ...
      <ul className={`${isOpen ? 'right-0' : '-right-full'} bg-primary fixed top-0 w-full h-screen text-white transition-all flex flex-col justify-center items-center space-y-8 text-lg`}>
        ...
        {navigation.map((item, index) => (
          <li key={index}>
            {/* smooth={true} duration={10000} onClick={() => setIsOpen(false)} */}
            <Link to={item.href} activeClass='active' spy={true} offset={-200} className='hover:text-accent-hover transition-all duration-300 cursor-pointer'>{item.name}</Link>
          </li>
        ))}
      </ul>
    </nav>
